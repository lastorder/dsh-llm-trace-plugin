...
- **Never add a `prepare` script to `package.json`.** This package ships compiled `dist/` so a git install never needs to build, which means it never triggers pnpm's `allowBuilds` approval prompt — see the README's "Install" section. Adding `prepare` reopens that prompt for every future git install.
- **`dist/` stays committed and stays out of `.gitignore`.** It is not a throwaway build artifact here; it is the actual publish content, checked in specifically so a git-spec install has something to load. Never add it to `.gitignore` because it "looks generated."
