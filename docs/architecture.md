...
sse.ts (parse) ──frames──► sse-merge/ (reassemble) ──merged JSON──► wire-trace-view.ts ──renders──► json-view.ts
                                                                            │
                                              json-model.ts (flatten), format.ts (labels), api-client.ts (fetch)
```
