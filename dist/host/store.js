...
export function createWireTraceStore(options) {
    ...
    function listArchive(settings) {
        ...
        const promise = archive.list({
            ...
            if (archiveCache !== null && archiveCache.promise === promise)
                archiveCache = null;
            throw error;
        });
    ...
    return {
        ...
        async listAll(options) {
            ...
            for (const record of memory.page)
                merged.set(record.id, record);
        ...
        async stats() {
            if (archive === null)
                return { persistence: false, memory: records.length };
