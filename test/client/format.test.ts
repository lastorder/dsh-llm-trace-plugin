...
test('sessionLabel: null/undefined/empty sessionId reports "无 session"', () => {
  assert.equal(sessionLabel(null, 'current'), '无 session')
  assert.equal(sessionLabel(undefined, 'current'), '无 session')
  assert.equal(sessionLabel('', 'current'), '无 session')
})

test('sessionLabel: the current session is labeled "本 session"', () => {
  assert.equal(sessionLabel('session-abc', 'session-abc'), '本 session')
})

test('sessionLabel: a foreign session shows a short id, distinguishable from another foreign session', () => {
  const a = sessionLabel('session-aaaaaaaa', 'session-current')
  const b = sessionLabel('session-bbbbbbbb', 'session-current')
  assert.notEqual(a, b)
...
test('purposeLabel: known purposes get a human label, unknown purposes pass through, absent returns null', () => {
  assert.equal(purposeLabel('session-title'), '标题生成')
  assert.equal(purposeLabel('compaction'), '上下文压缩')
  assert.equal(purposeLabel('custom-purpose'), 'custom-purpose')
...
test('stepBadge: a purposed call always reports "aux", regardless of turn/step', () => {
  const badge = stepBadge({ purpose: 'session-title', turn: 1, step: 0, attributed: true })
...
test('stepBadge: an ordinary call with turn and step reports "T{turn}·S{step}"', () => {
  const badge = stepBadge({ purpose: null, turn: 2, step: 3, attributed: true })
...
test('stepBadge: a turn with no step (between steps) still reports the turn alone', () => {
  const badge = stepBadge({ purpose: null, turn: 2, step: null, attributed: true })
  assert.equal(badge.kind, 'step')
  assert.equal(badge.text, 'T2')
})

test('stepBadge: no turn at all reports "无归属", distinguishing attributed:false from attributed:true', () => {
  const unattributed = stepBadge({ purpose: null, turn: null, step: null, attributed: false })
  assert.equal(unattributed.kind, 'none')
  assert.match(unattributed.title, /没有经过 ctx\.llm/)

  const attributedButNoTurn = stepBadge({ purpose: null, turn: null, step: null, attributed: true })
